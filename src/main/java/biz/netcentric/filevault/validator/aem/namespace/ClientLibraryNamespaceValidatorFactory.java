...
public class ClientLibraryNamespaceValidatorFactory extends AbstractPatternSettingsValidatorFactory {

    public ClientLibraryNamespaceValidatorFactory() {
        super("netcentric-clientlibrary-namespace", "allowedCategoryPatterns", false);
    }

    @Override
    protected Validator createValidator(
            @NotNull Set<Pattern> patterns, @NotNull ValidationContext context, @NotNull ValidatorSettings settings) {
        return new ClientLibraryNamespaceValidator(settings.getDefaultSeverity(), patterns);
    }
}
