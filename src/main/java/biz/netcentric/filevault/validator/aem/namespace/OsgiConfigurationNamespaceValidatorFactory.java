...
public class OsgiConfigurationNamespaceValidatorFactory extends AbstractPatternSettingsValidatorFactory {

    public OsgiConfigurationNamespaceValidatorFactory() {
        super("netcentric-osgiconfig-namespace", "allowedPidPatterns", true);
    }

    @Override
    protected Validator createValidator(
            @NotNull Set<Pattern> patterns, @NotNull ValidationContext context, @NotNull ValidatorSettings settings) {
        Set<Pattern> allowedFactoryPidNames =
                createPatternsFromCommaSeparatedString(settings.getOptions().get("allowedFactoryPidNames"));
        return new OsgiConfigurationNamespaceValidator(
                settings.getDefaultSeverity(),
                patterns,
                allowedFactoryPidNames,
                Boolean.parseBoolean(settings.getOptions().get("restrictFactoryConfigurationsToAllowedPidPatterns")));
    }
}
